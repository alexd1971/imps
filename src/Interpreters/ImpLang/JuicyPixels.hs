...
module Interpreters.ImpLang.JuicyPixels where
...
instance Interpreter IO where
  onDecodeJpeg bs = do
    ...
    where
      (image, metadata) =
        case JP.decodeImageWithMetadata . toStrict $ bs of
          Right (img, meta) -> (JP.convertRGB8 img, meta)
          Left e -> error e
      orientation =
        case lookup TagOrientation . extractExifMetas $ metadata of
          Just (ExifShort v) ->
            case v of
              3 -> UpSideDown
              6 -> CCW90
  ...
  onDecodePng bs = return image
    where
      image =
        case JP.decodeImage . toStrict $ bs of
  ...
  onResizeImage size image = do
    ...
    where
      newSize =
        calculateNewSize (JP.imageWidth image, JP.imageHeight image) size
